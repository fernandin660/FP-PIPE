...
export async function POST(request: Request) {
  ...
  const corpo = (await request.json().catch(() => null)) as { campanhaId?: unknown; instrucoes?: unknown; assunto?: unknown; corpo?: unknown } | null;
  ...
  const { data: campanha } = await supabase
    .from("campanhas")
    .select("id, nome, geracoes_usadas, status")
    .eq("id", campanhaId)
...
Gere um assunto de até 60 caracteres e um corpo de até 150 palavras. Use as variáveis {nome}, {empresa} e {cargo} quando ajudarem. Não invente dados. NÃO use placeholders como [Seu Nome], [Seu Contato], [Nome da Empresa] ou colchetes. Finalize usando o nome de quem envia informado acima. Retorne somente JSON no formato {"assunto":"...","corpo":"..."}.`;
  ...
  const { data: atualizada, error } = await supabase
    .from("campanhas")
    .update({ assunto: gerado.assunto.trim(), corpo: gerado.corpo.trim(), geracoes_usadas: novaGeracao, status: "pronta", atualizado_em: new Date().toISOString() })
    .eq("id", campanhaId)
    .eq("organizacao_id", orgId)
    .select("id, nome, assunto, corpo, geracoes_usadas, status")
    .single();
  if (error || !atualizada) {
